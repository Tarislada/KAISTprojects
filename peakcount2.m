%function [countmat, areamat] = peakcount2(datamat, class1, class2, class3, class4, class5, class6)
% CK, 2022.01
...
% Area calculation is under each counting line. change if needed.
    ...
    for i = 1:leng
		[~,tmpindice]=findpeaks(analmat(:,i),'MinPeakHeight',1,'MinPeakProminence',0.8, 'Threshold', 0.1,'MinPeakDistance',0.5);  % find peaks in each cells with minimum heigth and prominence(how much it stands out from vicinity
% 		[~,tmpindice]=findpeaks(analmat(:,i));
		...
		for ii = 1:size(tmp5,1)
			tmpval5 = find(timestamp<=tmp5(ii,1) & timestamp>=tmp5(ii,2));
			tmparray5 = [tmparray5; tmpval5];
		end
		count5 = sum(ismember(tmparray5,tmpindice));
		area5 = trapz(rawtime(tmparray5),abs(analmat(tmparray5,i)));

		for ii = 1:size(tmp6,1)
			tmpval6 = find(timestamp<=tmp6(ii,1) & timestamp>=tmp6(ii,2));
			tmparray6 = [tmparray6; tmpval6];
		end
		count6 = sum(ismember(tmparray6,tmpindice));
		area6 = trapz(rawtime(tmparray6),abs(analmat(tmparray6,i)));
